...
if(isset($_POST['name']) && isset($_POST['pass']) && isset($_POST['email']) )
{
		...
		else
		{
			...
			$Requette ="SELECT * FROM account";
			$res = mysqli_query($mysqli, $Requette);
			$userid=mysqli_num_rows($res);
			...
			if( $GM == 1)
			{
				$Requette = "INSERT INTO account(ID,PASSWORD,IDState, LastLogonTime, BlockTime,Logon,Sex,School,Event,MannerFlag,Event1,Event2 ,ChannelFlag,RegDate, NICK, UserName, UserIp, ServerID, LogonCount, capability)
				VALUES('".$Login."', '".$Password."',0, '".$Date."',0,0,0,0,0,0,0,0,0, '".$Date."', '".$userid."', '".$userid."', '127.0.0.1', 0, 1 , ".$GM.");";
			}
			else
			{
				
				$Requette = "INSERT INTO account(ID,PASSWORD,IDState, LastLogonTime, BlockTime,Logon,Sex,School,Event,MannerFlag,Event1,Event2 ,ChannelFlag,RegDate, NICK, UserName, UserIp, ServerID, LogonCount)
				VALUES('".$Login."', '".$Password."',0, '".$Date."',0,0,0,0,0,0,0,0,0, '".$Date."', '".$userid."', '".$userid."', '127.0.0.1', 0, 1 );";
			}
			...
			//Pangya tutorial
			$Requette = "INSERT INTO tutorial(UID, Rookie, Beginner, Advancer) VALUES('".$userid."',0,0,0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya macro
			$Requette = "INSERT INTO pangya_user_macro(UID, Macro1, Macro2, Macro3, Macro4, Macro5, Macro6, Macro7, Macro8, Macro9, Macro10) VALUES('".$userid."','Macro1','Macro2','Macro3','Macro4','Macro5','Macro6','Macro7','Macro8','Macro9','Macro10')";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Récompense
			$Requette = "INSERT INTO pangya_attendance_reward(UID, counter, item_typeid_now, item_qntd_now, item_typeid_after, item_qntd_after, login, reg_date) VALUES('".$userid."',0,0,0,0,0,0,'".$Date."')";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Scratchy
			$Requette = "INSERT INTO pangya_scratchy_prob_sec(UID,scratchy_sec) VALUES('".$userid."',0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Black pappel
			$Requette = "INSERT INTO black_papel_prob_sec(UID,probabilidade) VALUES('".$userid."',0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Tiki
			$Requette = "INSERT INTO pangya_tiki_points(UID, Tiki_Points, REG_DATE, MOD_DATE) VALUES('".$userid."',0,'".$Date."','".$Date."')";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Room
			$Requette = "INSERT INTO pangya_myroom(UID, senha, public_lock, state) VALUES('".$userid."',NULL,0,0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Casier
			$Requette = "INSERT INTO pangya_dolfini_locker(UID, senha, pang, locker) VALUES('".$userid."',NULL,0,0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Assistant
			$Requette = "INSERT INTO pangya_assistente(UID,Assist) VALUES('".$userid."',0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
			...
			//Pangya Grand Zodiac
			$Requette = "INSERT INTO pangya_grand_zodiac_pontos(UID,pontos) VALUES('".$userid."',0)";
			$res = mysqli_query($mysqli, $Requette);
			if(!$res && ERROR_USER > 0)
			{
				die("Mysql Error :".mysqli_error($mysqli));
